...
The main challenge is verifying the triangle inequality; the other axioms are straightforward. For all the following exercises, you may assume that $n = 2$ so that $\vec{x} = (x_1, x_2)$. The proofs extend to general $n$ by induction.
...
- Prove that if $\|x\|'$ and $\|x\|''$ are norms, then $\|x\|''' = \|x\|' + \|x\|''$ is also a norm.^[Because of these closure properties, the set of all norms forms a **convex cone**—it's closed under positive scaling and addition, but not subtraction.]
:::
...
**Exercise 4:** For data $g_1 = -2, g_2 = -1, g_3 = 0, g_4 = 1, g_5 = 4$, find the OLS estimate $\widehat{g}_2$.
...
**Exercise 6:** For data $g_1 = -2, g_2 = -1, g_3 = 0, g_4 = 1, g_5 = 4$, find the LAD estimate $\widehat{g}_1$.

**Exercise 7:** For sorted data $g_1 \leq g_2 \leq \cdots \leq g_n$ with $n$ odd, find $\widehat{g}_1$.

**Exercise 8:** For sorted data with $n$ even, find $\widehat{g}_1$. (The answer may not be unique!)
:::
...
::: {.callout-note}
## Exercises: $L^\infty$ Norm

**Exercise 9:** Prove that $\lim_{p \to \infty} \|\vec{x}\|_p = \max(|x_1|, \ldots, |x_n|)$.
...
**Exercise 11:** For data $g_1 = -2, g_2 = -1, g_3 = 0, g_4 = 1, g_5 = 4$, find $\widehat{g}_\infty$.

**Exercise 12:** For sorted data $g_1 \leq g_2 \leq \cdots \leq g_n$, find a formula for $\widehat{g}_\infty$.
